/**
 * Test: NA relay message server
 *
 * Same message exchange as test_libp2p_msg_server but over a relayed
 * connection (tcp,relay transport).
 *
 * 1. Initialize as listener with relay transport
 * 2. Write self address (circuit address) to file for client
 ...
 * 6. Finalize
 *
 * Usage: test_libp2p_relay_server [-p <protocol>]
 */
...
main(int argc, char *argv[])
{
    ...
    printf("Server: initializing with protocol '%s'...\n", protocol);
    state.na_class = NA_Initialize(info_string, true);
    ...
    /* Pre-fill with "relay:" hint so NA_Addr_to_string returns circuit address */
    memcpy(addr_str, "relay:", 6);
